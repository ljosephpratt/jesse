...
def test_can_add_new_ticker():
    ...
    np.testing.assert_equal(
        store.tickers.get_tickers("Sandbox", "BTC-USD"), np.zeros((0, 5))
    )
    ...
    np.testing.assert_equal(
        store.tickers.get_tickers("Sandbox", "BTC-USD"), np.array([t1, t2])
    )
...
def test_get_current_and_past_ticker():
    ...
    np.testing.assert_equal(
        store.tickers.get_tickers("Sandbox", "BTC-USD"), np.array([t1, t2, t3, t4])
    )
