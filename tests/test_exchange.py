...
def test_negative_balance_validation_for_futures_market():
    ...
    with pytest.raises(InsufficientMargin):
        single_route_backtest(
            "TestInsufficientMargin1", is_futures_trading=True, leverage=1
        )
    # but with more leverage, it should work
    single_route_backtest(
        "TestInsufficientMargin1", is_futures_trading=True, leverage=2
    )

    # (LIMIT order)
    with pytest.raises(InsufficientMargin):
        single_route_backtest(
            "TestInsufficientMargin2", is_futures_trading=True, leverage=1
        )
    # but with more leverage, it should work
    single_route_backtest(
        "TestInsufficientMargin2", is_futures_trading=True, leverage=2
    )

    # short-version (STOP order)
    with pytest.raises(InsufficientMargin):
        single_route_backtest(
            "TestInsufficientMargin3", is_futures_trading=True, leverage=1
        )
    # but with more leverage, it should work
    single_route_backtest(
        "TestInsufficientMargin3", is_futures_trading=True, leverage=2
    )
...
def test_available_margin_for_futures_market():
    ...
    """
    single_route_backtest(
        "TestFuturesExchangeAvailableMargin", is_futures_trading=True, leverage=2
    )
