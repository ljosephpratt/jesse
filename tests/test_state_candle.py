...
def set_up():
    ...
    router.set_data_candles(
        [{"exchange": "Sandbox", "symbol": "BTC-USD", "timeframe": "5m"}]
    )
...
def test_can_add_new_candle():
    ...
    np.testing.assert_equal(
        store.candles.get_candles("Sandbox", "BTC-USD", "1m"), np.zeros((0, 6))
    )

    c1 = fake_candle()
    store.candles.add_candle(c1, "Sandbox", "BTC-USD", "1m")
    np.testing.assert_equal(
        store.candles.get_candles("Sandbox", "BTC-USD", "1m")[0], c1
    )
    ...
    np.testing.assert_equal(
        store.candles.get_candles("Sandbox", "BTC-USD", "1m"), np.array([c1, c2])
    )
...
def test_can_update_candle():
    ...
    store.candles.add_candle(c1, "Sandbox", "BTC-USD", "1m")
    np.testing.assert_equal(
        store.candles.get_current_candle("Sandbox", "BTC-USD", "1m"), c1
    )
    ...
    store.candles.add_candle(c2, "Sandbox", "BTC-USD", "1m")
    np.testing.assert_equal(
        store.candles.get_current_candle("Sandbox", "BTC-USD", "1m"), c2
    )
