...
def test_forming_candles():
    ...
    data_routes = [{"symbol": "BTC-USDT", "timeframe": timeframes.MINUTE_15}]
    ...
    assert len(
        store.candles.get_candles(exchanges.SANDBOX, "BTC-USDT", timeframes.MINUTE_5)
    ) == math.ceil(1382 / 5)
    assert len(
        store.candles.get_candles(exchanges.SANDBOX, "BTC-USDT", timeframes.MINUTE_15)
    ) == math.ceil(1382 / 15)
    ...
    router.set_routes(
        [
            ...
            },
            {
                "exchange": exchanges.SANDBOX,
                "symbol": "ETH-USDT",
                "timeframe": timeframes.MINUTE_30,
                "strategy": "Test02",
            },
        ]
    )
...
def test_strategy_properties():
    ...
    for r in router.routes:
        ...
        np.testing.assert_equal(
            s.candles, store.candles.get_candles(r.exchange, r.symbol, r.timeframe)
        )
...
def test_reduce_only_market_orders():
    single_route_backtest(
        "TestReduceOnlyMarketOrders", is_futures_trading=True, leverage=1
    )
