...
def test_convert_number():
    ...
    assert (
        jh.convert_number(old_max, old_min, new_max, new_min, old_value)
        == 0.5443037974683544
    )
...
def test_get_config(monkeypatch):
    ...
    assert (
        jh.get_config("env.exchanges.Binance Futures.settlement_currency", "USDT")
        == "BUSD"
    )
...
def test_orderbook_insertion_index_search():
    ...
    assert jh.orderbook_insertion_index_search(
        descending_arr, [77, 2], ascending=False
    ) == (False, 0)
    assert jh.orderbook_insertion_index_search(
        descending_arr, [2, 2], ascending=False
    ) == (True, 4)
    assert jh.orderbook_insertion_index_search(
        descending_arr, [65, 2], ascending=False
    ) == (False, 1)
    assert jh.orderbook_insertion_index_search(
        descending_arr, [1, 2], ascending=False
    ) == (False, 5)


def test_orderbook_trim_price():
    # bids
    assert jh.orderbook_trim_price(101.12, False, 0.1) == 101.1
    assert jh.orderbook_trim_price(101.1, False, 0.1) == 101.1

    assert jh.orderbook_trim_price(10.12, False, 0.01) == 10.12
    assert jh.orderbook_trim_price(10.1, False, 0.01) == 10.1
    assert jh.orderbook_trim_price(10.122, False, 0.01) == 10.12
    assert jh.orderbook_trim_price(1.1223, False, 0.001) == 1.122

    # asks
    assert jh.orderbook_trim_price(101.12, True, 0.1) == 101.2
    assert jh.orderbook_trim_price(101.1, True, 0.1) == 101.1
    assert jh.orderbook_trim_price(10.12, True, 0.01) == 10.12
    assert jh.orderbook_trim_price(10.122, True, 0.01) == 10.13
    assert jh.orderbook_trim_price(1.1223, True, 0.001) == 1.123
...
def test_relative_to_absolute():
    ...
    assert jh.relative_to_absolute("tests/test_helpers.py") == str(
...
def test_round_qty_for_live_mode():
    ...
    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([0]), 1), np.array([0.1])
    )

    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([0]), 2), np.array([0.01])
    )

    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([0]), 3), np.array([0.001])
    )
    ...
    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([102]), -2), np.array([100])
    )
    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([123]), -2), np.array([100])
    )
    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([163]), -2), np.array([100])
    )
    np.testing.assert_equal(
        jh.round_qty_for_live_mode(np.array([1263]), -2), np.array([1200])
    )
...
def test_merge_dicts():
    client = {"extra": {"name": "Saleh", "new_key": 12}, "age": 28}

    server = {
        "extra": {"name": "Ocean", "water": 100},
    }

    expected_result = {
        "age": 28,
        "extra": {"name": "Ocean", "water": 100, "new_key": 12},
    }
...
def test_convert_to_env_name():
    assert (
        jh.convert_to_env_name("Testnet Binance Futures") == "TESTNET_BINANCE_FUTURES"
    )
...
def test_is_price_near():
    assert jh.is_price_near(0.007386, 0.007385)
    assert jh.is_price_near(0.007386, 0.007396) is False
    assert jh.is_price_near(0.0250, 0.0249) is False
    assert jh.is_price_near(60000, 60000)
    assert jh.is_price_near(60000, 60000.1)
    assert jh.is_price_near(60000, 60100) is False
    assert jh.is_price_near(30000, 30005) is False
    assert jh.is_price_near(30000, 30002)
    assert jh.is_price_near(30000, 29800) is False
    assert jh.is_price_near(200, 200.01)
    assert jh.is_price_near(20, 20.001)
    assert jh.is_price_near(0.0014458, 0.0014458 * 1.05) is False
    assert jh.is_price_near(0.0014458, 0.0014458 * 1.10) is False
