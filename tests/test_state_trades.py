...
def test_can_add_add_trade():
    ...
    np.testing.assert_equal(
        store.trades.get_trades("Sandbox", "BTC-USD"), np.zeros((0, 6))
    )
    ...
    np.testing.assert_equal(
        store.trades.get_current_trade("Sandbox", "BTC-USD"),
        np.array(
            [
                jh.now_to_timestamp(),
                # price
    ...
    # test get_past_trade
    np.testing.assert_equal(
        store.trades.get_past_trade("Sandbox", "BTC-USD", 1),
        np.array(
            [
                jh.now_to_timestamp(),
                # price
    ...
    # test get_trades
    np.testing.assert_equal(
        store.trades.get_trades("Sandbox", "BTC-USD"),
        np.array(
            [
                [
                    jh.now_to_timestamp(),
