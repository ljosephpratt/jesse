...
def store_candle_into_db(
    exchange: str, symbol: str, timeframe: str, candle: np.ndarray, on_conflict="ignore"
) -> None:
...
def store_ticker_into_db(exchange: str, symbol: str, ticker: np.ndarray) -> None:
    ...
    def async_save() -> None:
        ...
        print(
            jh.color(
                f'ticker: {jh.timestamp_to_time(d["timestamp"])}-{exchange}-{symbol}: {ticker}',
                "yellow",
            )
        )
...
def store_order_into_db(order) -> None:
    ...
    def async_save() -> None:
        ...
        if jh.is_debugging():
            logger.info(
                f"Stored the executed order record for {order.exchange}-{order.symbol} into database."
            )
...
def fetch_candles_from_db(
    exchange: str, symbol: str, timeframe: str, start_date: int, finish_date: int
) -> tuple:
    from jesse.models.Candle import Candle
