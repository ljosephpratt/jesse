...
def set_config(conf: dict) -> None:
    ...
    if jh.is_backtesting() or jh.is_live():
        ...
        for key, e in conf["exchanges"].items():
            ...
            if config["env"]["exchanges"][e["name"]]["type"] == "futures":
                # 1x, 2x, 10x, 50x, etc. Enter as integers
                config["env"]["exchanges"][e["name"]]["futures_leverage"] = int(
                    e.get("futures_leverage", 1)
                )
                # accepted values are: 'cross' and 'isolated'
                config["env"]["exchanges"][e["name"]]["futures_leverage_mode"] = e.get(
                    "futures_leverage_mode", "cross"
                )
    ...
    if jh.is_live():
        ...
        config["env"]["data"]["generate_candles_from_1m"] = conf[
