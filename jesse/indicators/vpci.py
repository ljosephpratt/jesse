...
def vpci(
    ...
    vwma_long = sma(candles[:, 2] * candles[:, 5], long_range, sequential=True) / sma(
        candles[:, 5], long_range, sequential=True
    )
    VPC = vwma_long - sma(candles[:, 2], long_range, sequential=True)

    vwma_short = sma(candles[:, 2] * candles[:, 5], short_range, sequential=True) / sma(
        candles[:, 5], short_range, sequential=True
    )
    VPR = vwma_short / sma(candles[:, 2], short_range, sequential=True)

    VM = sma(candles[:, 5], short_range, sequential=True) / sma(
        candles[:, 5], long_range, sequential=True
    )
    VPCI_val = VPC * VPR * VM

    VPCIS = sma(VPCI_val * candles[:, 5], short_range, sequential=True) / sma(
        candles[:, 5], short_range, sequential=True
    )
