...
def vwma(
    ...
    vwma_vals[:period] = cumsum_weighted[:period] / np.where(
        cumsum_volume[:period] == 0, 1, cumsum_volume[:period]
    )

    # For indices with a complete period, use the rolling difference of cumulative sums
    vwma_vals[period:] = (
        cumsum_weighted[period:] - cumsum_weighted[:-period]
    ) / np.where(
        (cumsum_volume[period:] - cumsum_volume[:-period]) == 0,
        1,
        (cumsum_volume[period:] - cumsum_volume[:-period]),
    )
