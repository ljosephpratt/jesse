...
def _fisher_transform(high: np.ndarray, low: np.ndarray, period: int) -> tuple:
    ...
    for i in range(period, length):
        ...
        if max_h - min_l == 0:
            ...
            value0 = (
                0.33 * 2 * ((mid_price[i] - min_l) / (max_h - min_l) - 0.5)
                + 0.67 * value1
            )
...
def fisher(
    candles: np.ndarray, period: int = 9, sequential: bool = False
) -> FisherTransform:
    """
    ...
    fisher_val, fisher_signal = _fisher_transform(
        np.ascontiguousarray(candles[:, 3]), np.ascontiguousarray(candles[:, 4]), period
    )

    if sequential:
        return FisherTransform(
            same_length(candles, fisher_val), same_length(candles, fisher_signal)
        )
    else:
