...
def hull_suit(
    ...
    if mode_switch == "Hma":
        mode = wma(
            2 * wma(source, int(mode_len / 2), sequential=True)
            - wma(source, mode_len, sequential=True),
            round(mode_len**0.5),
            sequential=True,
        )
    elif mode_switch == "Ehma":
        mode = ema(
            2 * ema(source, int(mode_len / 2), sequential=True)
            - ema(source, mode_len, sequential=True),
            round(mode_len**0.5),
            sequential=True,
        )
    elif mode_switch == "Thma":
        mode = wma(
            3 * wma(source, int(mode_len / 6), sequential=True)
            - wma(source, int(mode_len / 4), sequential=True)
            - wma(source, int(mode_len / 2), sequential=True),
            int(mode_len / 2),
            sequential=True,
        )
