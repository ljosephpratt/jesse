...
def stiffness(
    ...
    bound_stiffness = sma(source, ma_length, sequential=True) - 0.2 * stddev(
        source, ma_length, sequential=True
    )
    sum_above_stiffness = _count_price_exceed_series(
        source, bound_stiffness, stiff_length
    )
