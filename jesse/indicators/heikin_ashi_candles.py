...
def ha_fast(source):
    ...
    for i in range(1, ha_candles.shape[0]):
        ...
        ha_candles[i][CLOSE] = (
            source[i][OPEN] + source[i][CLOSE] + source[i][HIGH] + source[i][LOW]
        ) / 4
        ...
        ha_candles[i][HIGH] = max(
            [source[i][HIGH], ha_candles[i][OPEN], ha_candles[i][CLOSE]]
        )
        ha_candles[i][LOW] = min(
            [source[i][LOW], ha_candles[i][OPEN], ha_candles[i][CLOSE]]
        )
