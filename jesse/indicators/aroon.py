...
def aroon(candles: np.ndarray, period: int = 14, sequential: bool = False) -> AROON:
    ...
    if sequential:
        ...
        if len(highs) >= period + 1:
            ...
            windows_high = np.lib.stride_tricks.sliding_window_view(
                highs, window_shape=period + 1
            )
            windows_low = np.lib.stride_tricks.sliding_window_view(
                lows, window_shape=period + 1
            )
