...
def store_notification_api_keys(name: str, driver: str, fields: dict) -> JSONResponse:
    from jesse.services.db import database
...
def delete_notification_api_keys(notification_api_key_id: str) -> JSONResponse:
    ...
    try:
        # delete the record
        NotificationApiKeys.delete().where(
            NotificationApiKeys.id == notification_api_key_id
        ).execute()
    except Exception as e:
        database.close_connection()
        return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
