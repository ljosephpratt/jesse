...
def get_fitness(
    ...
    if training_data_metrics["total"] > 5:
        ...
        # elif jh.is_debugging():
            ...
            logger.log_optimize_mode(
                f"Name: {dna} - DNA is usable => {ratio_config}: {round(ratio, 2)}, total: {training_data_metrics['total']}, PNL%: {round(training_data_metrics['net_profit_percentage'], 2)}%, win-rate: {round(training_data_metrics['win_rate'] * 100, 2)}%"
            )
...
def get_and_add_fitness_to_the_bucket(
    ...
    try:
        ...
        if all(dna_tuple[0] != dna for dna_tuple in dna_bucket):
            ...
            dna_bucket.append(
                (dna, fitness_score, fitness_log_training, fitness_log_testing)
            )
        else:
