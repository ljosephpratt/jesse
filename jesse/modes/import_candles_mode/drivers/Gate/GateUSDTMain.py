...
class GateUSDTMain(CandleExchange):
    def __init__(self, name: str, rest_endpoint: str) -> None:
        super().__init__(
            name=name, count=200, rate_limit_per_second=10, backup_exchange_class=None
        )
        self.name = name
        self.limit = 2000
    ...
    def get_starting_time(self, symbol: str) -> int:
        ...
        if response.json() == []:
            raise exceptions.InvalidSymbol(
                "Exchange does not support the entered symbol. Please enter a valid symbol."
            )
        ...
        for attempt in range(max_retries):
            try:
                response = self.session.get(
                    f"{self.endpoint}/usdt/candlesticks", params=payload, timeout=30
                )
                self.validate_response(response)
                break
            except (
