...
def run(
    ...
    for i in range(candles_count):
        ...
        if already_exists:
            ...
            if temp_end_timestamp > jh.now_to_timestamp():
                temp_end_timestamp = (
                    arrow.utcnow().floor("minute").int_timestamp * 1000 - 60000
                )
            ...
            if not len(candles) or time_diff < 0 or time_diff > 60 * 100:
                ...
                if temp_start_timestamp > first_existing_timestamp:
                    ...
                    temp_existing_time = jh.timestamp_to_time(first_existing_timestamp)[
                        :10
                    ]
                    ...
                    if running_via_dashboard:
                        sync_publish("alert", {"message": msg, "type": "info"})
                    else:
                        print(msg)
                    run(
                        client_id,
            ...
            candles = _fill_absent_candles(
                candles, temp_start_timestamp, temp_end_timestamp
            )
    ...
    if running_via_dashboard:
        ...
        sync_publish("alert", {"message": success_text, "type": "success"})
...
def _get_candles_from_backup_exchange(
    ...
    already_exists = (
        len(backup_candles) == (end_timestamp - start_timestamp) / 60_000 + 1
    )
    if already_exists:
    ...
    days_count = jh.date_diff_in_days(
        jh.timestamp_to_arrow(start_timestamp), jh.timestamp_to_arrow(end_timestamp)
    )
...
def _fill_absent_candles(
    temp_candles: List[Dict[str, Union[str, Any]]],
    start_timestamp: int,
    end_timestamp: int,
) -> List[Dict[str, Union[str, Any]]]:
    if not temp_candles:
