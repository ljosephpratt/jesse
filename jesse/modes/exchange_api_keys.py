...
def get_exchange_api_keys() -> JSONResponse:
    ...
    return JSONResponse({"data": api_keys}, status_code=200)
...
def store_exchange_api_keys(
    ...
    if exchange not in live_trading_exchanges:
        return JSONResponse(
            {"status": "error", "message": f"Invalid exchange: {exchange}"},
            status_code=400,
        )
    ...
    try:
        ...
        exchange_api_key: ExchangeApiKeys = ExchangeApiKeys.create(
            ...
            general_notifications_id=(
                general_notifications_id if general_notifications_id else None
            ),
            error_notifications_id=(
                error_notifications_id if error_notifications_id else None
            ),
        )
...
def delete_exchange_api_keys(exchange_api_key_id: str) -> JSONResponse:
    ...
    try:
        # delete the record
        ExchangeApiKeys.delete().where(
            ExchangeApiKeys.id == exchange_api_key_id
        ).execute()
    except Exception as e:
        database.close_connection()
        return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    ...
    return JSONResponse(
        {"status": "success", "message": "API key has been deleted successfully."},
        status_code=200,
    )
