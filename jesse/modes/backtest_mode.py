    ...
    if candles is None:
        try:
            warmup_candles, candles = load_candles(
                jh.date_to_timestamp(start_date), jh.date_to_timestamp(finish_date)
            )
    ...
    if result and not jh.should_execute_silently():
        ...
        if chart:
            ...
            sync_publish(
                ...
                _get_add_line_to_candle_chart(),
                compression=True,
            )
            sync_publish(
                "add_extra_line_chart", _get_add_extra_line_chart(), compression=True
            )
            sync_publish(
                "add_horizontal_line_to_candle_chart",
                _get_add_horizontal_line_to_candle_chart(),
                compression=True,
            )
            sync_publish(
...
def _handle_warmup_candles(warmup_candles: dict, start_date: str) -> None:
    try:
        for c in config["app"]["considering_candles"]:
            exchange, symbol = c[0], c[1]
            inject_warmup_candles_to_store(
                warmup_candles[jh.key(exchange, symbol)]["candles"], exchange, symbol
            )
        ...
        if match:
            ...
            warmup_start_timestamp = jh.date_to_timestamp(start_date) - (
                warmup_minutes * 60_000
            )
            ...
            for timeframe in config["app"]["considering_timeframes"]:
                ...
                if (i + 1) % count == 0:
                    generated_candle = generate_candle_from_one_minutes(
                        timeframe, candles[j]["candles"][(i - (count - 1)) : (i + 1)]
                    )
...
def _simulate_new_candles(candles: dict, candle_index: int, candles_step: int) -> None:
    ...
    for j in candles:
        ...
        _simulate_price_change_effect_multiple_candles(short_candles, exchange, symbol)
...
def _update_all_routes_a_partial_candle(
    ...
    for route in router.all_formatted_routes:
        ...
        number_of_needed_candles = (
            int(storable_temp_candle[0] % (tf_minutes * 60_000) // 60000) + 1
        )
        candles_1m = store.candles.get_candles(exchange, symbol, "1m")[
            -number_of_needed_candles:
        ]
        generated_candle = generate_candle_from_one_minutes(
            timeframe, candles_1m, accept_forming_candles=True
        )
        store.candles.add_candle(
            generated_candle,
            exchange,
...
def _execute_routes(candle_index: int, candles_step: int) -> None:
    ...
    for r in router.routes:
        ...
        if r.timeframe == timeframes.MINUTE_1:
            ...
            if jh.is_debuggable("trading_candles"):
                print_candle(
                    store.candles.get_current_candle(r.exchange, r.symbol, r.timeframe),
                    False,
                    r.symbol,
                )
            r.strategy._execute()
    ...
    for i in range(len(short_candles)):
        ...
        if len(included_orders) == 1:
            ...
            if is_red:
                ...
                sorted_orders += sorted(above_open, key=lambda o: o.price) + sorted(
                    below_open, key=lambda o: o.price, reverse=True
                )
            else:
                sorted_orders += sorted(
                    below_open, key=lambda o: o.price, reverse=True
                ) + sorted(above_open, key=lambda o: o.price)
