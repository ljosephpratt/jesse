...
def get_candles(exchange: str, symbol: str, timeframe: str):
    ...
    start_date = jh.get_candle_start_timestamp_based_on_timeframe(
        timeframe, warmup_candles_num
    )

    # fetch value of generate_candles_from_1m fresh from the database
    o = Option.get(Option.type == "config")
    generate_candles_from_1m: bool = json.loads(o.json)["live"][
    ...
    candles = np.array(
        fetch_candles_from_db(
            exchange, symbol, timeframe_to_fetch, start_date, finish_date
        )
    )
            ...
            for i in range(len(candles)):
                if (i + 1) % one_min_count == 0:
                    bigger_candle = generate_candle_from_one_minutes(
                        timeframe, candles[(i - (one_min_count - 1)) : (i + 1)], True
                    )
