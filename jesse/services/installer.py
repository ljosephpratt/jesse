...
def install(is_live_plugin_already_installed: bool, strict: bool):
    ...
    if "LICENSE_API_TOKEN" not in ENV_VALUES:
        if strict:
            print(
                "ERROR: You need to set the LICENSE_API_TOKEN environment variable in your .env"
            )
        return
    ...
    if not ENV_VALUES["LICENSE_API_TOKEN"]:
        if strict:
            print(
                "No license API token set. Please set the LICENSE_API_TOKEN environment variable to continue. If you don't have one yet, create one at https://jesse.trade/user/api-tokens"
            )
        return
    else:
    ...
    if is_arm and (os_name in ["linux", "windows"]):
        raise NotImplementedError(
            f"ARM versions of {os_name} are not supported. If you need them, send a request to contact@jesse.trade"
        )
