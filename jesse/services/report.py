...
def candles() -> dict:
    ...
    for e in router.routes:
        ...
        candle_keys.append(
            {"exchange": e.exchange, "symbol": e.symbol, "timeframe": e.timeframe}
        )
    ...
    for k in candle_keys:
        try:
            c = store.candles.get_current_candle(
...
def livetrade():
    ...
    routes = [
        {"symbol": r.symbol, "timeframe": r.timeframe, "strategy": r.strategy_name}
        for r in router.routes
    ]
...
def watch_list() -> List[List[Union[str, str]]]:
    ...
    for index, value in enumerate(watch_list_array):
        ...
        if not isinstance(value, tuple) or len(value) != 2:
            raise ValueError(
                "watch_list() must return a list of tuples with 2 values in each. Example: [(key1, value1), (key2, value2)]"
            )
