...
def _calculate_equity_curve(daily_balance, start_date, name: str, color: str):
    ...
    eq = [
        {"time": date.timestamp(), "value": balance, "color": color}
...
def equity_curve(benchmark: bool = False) -> list:
    ...
    result.append(
        _calculate_equity_curve(daily_balance, start_date, "Portfolio", colors[0])
    )
    ...
    if benchmark:
        ...
        for i, r in enumerate(router.routes):
            ...
            daily_balance_benchmark = (
                initial_balance * (1 + daily_returns / 100).cumprod()
            )
