...
def get_exchange_api_key(exchange_api_key: ExchangeApiKeys) -> dict:
    result = {
        ...
        "api_key": exchange_api_key.api_key[0:4]
        + "***...***"
        + exchange_api_key.api_key[-4:],
        "api_secret": exchange_api_key.api_secret[0:4]
        + "***...***"
        + exchange_api_key.api_secret[-4:],
        "created_at": exchange_api_key.created_at.isoformat(),
    }

    if isinstance(exchange_api_key.additional_fields) == str:
        exchange_api_key.additional_fields = json.loads(
            exchange_api_key.additional_fields
        )
...
def get_notification_api_key(
    api_key: NotificationApiKeys, protect_sensitive_data=True
) -> dict:
    result = {
        "id": str(api_key.id),
        "name": api_key.name,
