...
def candles_info(candles_array: np.ndarray) -> dict:
    period = (
        jh.date_diff_in_days(
            jh.timestamp_to_arrow(candles_array[0][0]),
            jh.timestamp_to_arrow(candles_array[-1][0]),
        )
        + 1
    )
...
def trades(
    trades_list: List[ClosedTrade], daily_balance: list, final: bool = True
) -> dict:
    starting_balance = 0
    current_balance = 0
    ...
    largest_winning_trade = (
        0 if total_winning_trades == 0 else winning_trades["PNL"].max()
    )
    ...
    expectancy = (0 if np.isnan(average_win) else average_win) * win_rate - (
        0 if np.isnan(average_loss) else average_loss
    ) * (1 - win_rate)
    ...
    max_dd = (
        np.nan if len(daily_return) < 2 else max_drawdown(daily_return).iloc[0] * 100
    )
    annual_return = (
        np.nan
        if len(daily_return) < 2
        else cagr(daily_return, periods=365).iloc[0] * 100
    )
    sharpe = (
        np.nan
        if len(daily_return) < 2
        else sharpe_ratio(daily_return, periods=365).iloc[0]
    )
    calmar = np.nan if len(daily_return) < 2 else calmar_ratio(daily_return).iloc[0]
    sortino = (
        np.nan
        if len(daily_return) < 2
        else sortino_ratio(daily_return, periods=365).iloc[0]
    )
    omega = (
        np.nan
        if len(daily_return) < 2
        else omega_ratio(daily_return, periods=365).iloc[0]
    )
    ...
    return {
        ...
        "expected_net_profit_every_100_trades": safe_convert(
...
def hyperparameters(routes_arr: list) -> list:
    ...
    for key in routes_arr[0].strategy.hp:
        hp.append([key, routes_arr[0].strategy.hp[key]])
    return hp
