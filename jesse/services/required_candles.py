...
def load_required_candles(
    exchange: str, symbol: str, start_date_str: str, finish_date_str: str
) -> np.ndarray:
    """
    ...
    finish_date = (
        jh.arrow_to_timestamp(arrow.get(finish_date_str, "YYYY-MM-DD")) - 60000
    )
    ...
    ) * jh.timeframe_to_one_minutes(max_timeframe)
    ...
    pre_start_date = (
        jh.timestamp_to_arrow(pre_start_date).floor("day").int_timestamp * 1000
    )
    ...
    if len(candles) < short_candles_count + 1:
        ...
        first_backtestable_timestamp = (
            first_existing_candle + (pre_finish_date - pre_start_date) + (60_000 * 1440)
        )
        first_backtestable_datetime = datetime.fromtimestamp(
...
def inject_required_candles_to_store(
