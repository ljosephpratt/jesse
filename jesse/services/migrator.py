    ...
    fields = [
        {
            ...
            "name": "timeframe",
            "type": CharField(index=False, null=True),
        },
        {
            "action": migration_actions.DROP_INDEX,
            "indexes": ("exchange", "symbol", "timestamp"),
        },
        {
            "action": migration_actions.ADD_INDEX,
            "indexes": ("exchange", "symbol", "timeframe", "timestamp"),
            "is_unique": True,
        },
    ]
...
def _migrate(migrator, fields, columns, table):
    for field in fields:
        if field["action"] in [
            migration_actions.ADD_INDEX,
            migration_actions.DROP_INDEX,
        ]:
            ...
            if field["action"] == migration_actions.DROP_INDEX:
                if already_exists:
                    migrate(migrator.drop_index(table, to_migrate_indexes_str))
            ...
            if column_name_exist:
                if field["action"] == migration_actions.ADD:
                    ...
                    migrate(migrator.drop_column(table, field["name"]))
                    print(
                        f"Successfully dropped '{field['name']}' column from the "
                        "{table}"
                        " table."
                    )
                    ...
                    print(
                        f"'{field['name']}' column successfully changed to {field['new_name']} in the '{table}' table."
                    )
                    ...
                    migrate(migrator.drop_not_null(table, field["name"]))
                    print(
                        f"'{field['name']}' column successfully updated to accept nullable values in the '{table}' table."
                    )
                elif field["action"] == migration_actions.DENY_NULL:
                    migrate(migrator.add_not_null(table, field["name"]))
                    print(
            ...
            # if column name doesn't not already exist
                ...
                if field["action"] == migration_actions.ADD:
                    migrate(migrator.add_column(table, field["name"], field["type"]))
                    print(
                        f"'{field['name']}' column successfully added to '{table}' table."
                    )
                else:
