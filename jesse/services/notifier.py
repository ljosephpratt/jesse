...
def start_notifier_loop():
    ...
    def handle_time():
        if len(MSG_QUEUE) > 0:
            ...
            notification_keys = (
                get_notification_api_key(
                    store.app.notifications_api_key, protect_sensitive_data=False
                )
                if store.app.notifications_api_key
                else None
            )
            ...
            if msg["type"] == "info":
                if msg["webhook"] is None and notification_keys:
                    if notification_keys["driver"] == "telegram":
                        _telegram(
                            msg["content"],
                            notification_keys["bot_token"],
                            notification_keys["chat_id"],
                        )
                    elif notification_keys["driver"] == "discord":
                        _discord(
                            msg["content"], webhook_address=notification_keys["webhook"]
                        )
...
def _custom_channel_notification(msg: dict):
    ...
    if webhook.startswith("https://hooks.slack.com"):
        ...
        raise ValueError(
            f"Custom Webhook {webhook}. seems to be neither a discord or slack webhook"
        )
