...
class Broker:
    def __init__(
        self, position: Position, exchange: str, symbol: str, timeframe: str
    ) -> None:
    ...
    def sell_at(self, qty: float, price: float) -> Union[Order, None]:
        ...
        return self.api.limit_order(
            self.exchange, self.symbol, abs(qty), price, sides.SELL, reduce_only=False
        )
    ...
    def buy_at(self, qty: float, price: float) -> Union[Order, None]:
        ...
        return self.api.limit_order(
            self.exchange, self.symbol, abs(qty), price, sides.BUY, reduce_only=False
        )

    def reduce_position_at(
        self, qty: float, price: float, current_price: float
    ) -> Union[Order, None]:
        self._validate_qty(qty)
        ...
        if jh.is_price_near(price, current_price):
            return self.api.market_order(
                self.exchange, self.symbol, qty, price, side, reduce_only=True
            )

        # LIMIT order
        elif (
            side == "sell" and self.position.type == "long" and price > current_price
        ) or (
        ...
        # STOP order
            ...
            return self.api.stop_order(
                self.exchange, self.symbol, abs(qty), price, side, reduce_only=True
            )
        else:
            raise OrderNotAllowed(
    ...
    def start_profit_at(
        ...
        return self.api.stop_order(
            self.exchange, self.symbol, abs(qty), price, side, reduce_only=False
        )
