...
def generate(name: str) -> JSONResponse:
    ...
    if exists:
        return JSONResponse(
            {"status": "error", "message": f'Strategy "{name}" already exists.'},
            status_code=409,
        )
    ...
    return JSONResponse({"status": "success", "message": path})
...
def get_strategies() -> JSONResponse:
    ...
    return JSONResponse({"status": "success", "strategies": directories})
...
def get_strategy(name: str) -> JSONResponse:
    ...
    if not exists:
        return JSONResponse(
            {"status": "error", "message": f'Strategy "{name}" does not exist.'},
            status_code=404,
        )
...
def save_strategy(name: str, content: str) -> JSONResponse:
    ...
    return JSONResponse(
        {"status": "success", "message": f'Strategy "{name}" has been saved.'}
    )
...
def delete_strategy(name: str) -> JSONResponse:
    ...
    return JSONResponse(
        {"status": "success", "message": f'Strategy "{name}" has been deleted.'}
    )
